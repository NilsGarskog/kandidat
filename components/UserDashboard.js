...
export default function UserDashboard() {
  ...
  async function handleAddTrip() {
    ...
    const newKey =
      Object.keys(trips).length === 0 ? 1 : Math.max(...Object.keys(trips)) + 1;
    setTrips({ ...trips, [newKey]: trip });
    const userRef = doc(
      db,
      "users",
    ...
    await setDoc(userRef, {
      Name: trip,
      arrDate: arrDate.format("YYYY-MM-DD"),
  ...
  return (
    ...
    text-xs sm:text-sm overflow-hidden'>
      ...
      {!loading && (
        <div className=" flex pb-20 pt-5 h-[60ch] pr-3 sm:h-[65ch] flex-wrap items-start overflow-y-auto pl-3 gap-5 w-full items-start content-start sm:justify-between justify-center justify-self-center">
          <>
            {Object.keys(trips)
              .reverse()
              .map((trip, i) => {
      ...
      <div className="  w-full flex justify-center -mt-20 sm:-mt-28 z-10 bg-gradient-to-t from-white h-[10ch] items-start ">
        <button
          onClick={() => handleButton()}
          className=" rounded-full bg-buttonGreen opacity-100 hover:opacity-80 duration-300 shadow-lg h-20 w-20 cursor-pointer"
        >
      ...
      >
        <div className="flex flex-col font-medium text-base items-center rounded-lg w-full">
          <i
            ...
            className="text-3xl fa-solid fa-xmark cursor-pointer absolute top-2 right-4 "
          ></i>
      ...
      text-base sm:text-lg text-slate-900 flex-1"
          ...
          <button
            className="border w-1/2 bg-buttonGreen opacity-100 hover:opacity-80 duration-300 text-black rounded-xl p-3 m-4"
            onClick={() => {
              handleAddTrip();
            }}
          >
