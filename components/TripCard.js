    ...
    return (
      <div className="flex flex-row items-center">
        <div className="sm:max-w-sm max-w-xs duration-300 hover:bg-gray-100 shadow-xl bg-white text-black rounded-xl items-center">
          <div className="flex items-center pr-40 pl-3 pt-4 pb-4">
            <Link href={`/${tripKey}`} className=" flex items-center">
              <div
                className={`${uniqueClassName} ml-3 h-20 w-20  rounded-full overflow-hidden`}
              >
                <img
                  className="w-full h-full object-cover"
                  src={
                    tripData.tripImageUrl?.[tripData.preferredImageIndex]
                      ?.urlThumb || "../img/placeholder-image.png"
                  }
                />
              </div>
              ...
              <div className="flex flex-col ml-4">
                ...
                <div className="font-light text-xs">
                  {dayjs(tripData.arrDate).format("D MMM YYYY")} →{" "}
                  {dayjs(tripData.depDate).format("D MMM YYYY")}
                </div>
              </div>
            </Link>
          </div>
        ...
        {allData.loading === false &&
          allData.tripData &&
          allData.tripData.tripImageUrl &&
          allData.tripData.tripImageUrl[tripData.preferredImageIndex] && (
            <Tooltip anchorSelect={`.${uniqueClassName}`} place="top" clickable>
              ...
              <Link
                href={
                  allData.tripData.tripImageUrl[tripData.preferredImageIndex]
                    .portfolioUrl +
                  "?utm_source=travel_planner_kandidat&utm_medium=referral"
                }
                target="_blank"
              >
                <u>
