...
export default function Modal(props) {
  ...
  const uploadImage = () => {
    ...
    uploadBytes(imageRef, imageUpload).then(() => {
      getDownloadURL(imageRef).then((url) => setUploadedImageUrl(url));
    ...
    if (imageUpload && selectedImage != null) {
      ...
      uploadBytes(imageRef, imageUpload).then(() => {
        getDownloadURL(imageRef).then((url) => {
          ...
          setDoc(
            userRef,
            { ProfileInfo: { ...profileInfo, ProfileImageURL: url } },
            { merge: true }
          );
    ...
    if (selectedImage == null) {
      ...
      if (docSnap.exists()) {
        ...
        if (profileInfo.ProfileImageURL) {
          ...
          await setDoc(
            userRef,
            { ProfileInfo: { ProfileImageURL: deleteField() } },
            { merge: true }
          );
  ...
  return ReactDom.createPortal(
    <div
      className="fixed border border-2 border-black left-auto right-0 sm:w-[100vh] w-[100vh] inset-0 bg-white text-slate-900 text-lg sm:text-xl flex flex-col"
              ...
              <div className="select-none">
                ...
                <div className="flex place-content-between ">
                  <div className="flex flex-col sm:justify-start sm:items-start justify-center items-center">
                    <div className="sm:text-2xl text-s sm:font-light font uppercase sm:pl-4 pl-0 pt-4">
